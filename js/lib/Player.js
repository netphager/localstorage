...
//	var EventEmitter 	= require('base/js/libs/eventemitter'),
//		Cayetano 		= require('base/js/Cayetano');
	...
	jukebox.Player.mute = false;

//	jukebox.Player.prototype = jukebox.Player.fn = new EventEmitter();
//	jukebox.Player.fn.extend = jukebox.Player.extend = Cayetano.extend;

//	jukebox.Player.extend({
//		events : {
//			'PLAYING_STARTED' 	: 'playing_started',
//			'PLAYING_ENDED'		: 'playing_ended',
//			'LOOPING_STARTED'	: 'looping_started',
//			'LOOPING_ENDED'		: 'looping_ended'
//		}
//	});
...
//				this.emit(jukebox.Player.events.LOOPING_STARTED);
				this.play(sound);
                this.loopingSound = sound;
                if ( this.isPlaying != null ){
                    this.isPlaying.loop = true;
                }

			},
			loopRemove : function(){
//                this.emit(jukebox.Player.events.LOOPING_ENDED);
                this.loopingSound = null;
				if(this.isPlaying) {
				 	this.isPlaying.loop = false;
                }
			},

			stopAll : function(){
				for( x in jukebox.Manager.__players){
					// jukebox.Manager.__players[x].unsetInterval();
					jukebox.Manager.__players[x].stop();
				}
				this.loopRemove();
				//this.clearQueue();
			},

			// unsetInterval : function(){
				// clearInterval(this.i);
			// },
	//
			// muteAll : function(){
				// for( x in jukebox.Manager.__players){
					// jukebox.Manager.__players[x].pause();
				// }
			// },

			setMute : function(){
				jukebox.Player.mute = true;
			...
			isMute: function(){
				return jukebox.Player.mute;
			},

			checkIfPlaying : function(){
				for( x in jukebox.Manager.__players){
					if(jukebox.Manager.__players[x].isPlaying)
							return true;
				}
				return false;
			},
			clearQueue : function(){
				while(!jukebox.Manager.queue.isEmpty()){
					jukebox.Manager.queue.dequeue();
				}
			},
		 ...
		 */
	//	play: function(pointer, enforce, callback) {
		play: function(pointer, callback ) {
			// alert(1)
;            if (!!(/android/i).test(navigator.userAgent) && this.context.readyState != 4) {
                return;
            }

			if(jukebox.Player.mute){
				return;
			}
            ...
            // fix for iOS, no play emptyLoop
            if ( pointer == 'emptyLoop' && jukebox.Manager.features.channels == 1 ) {
            	this.stop();
                return;
            }
			...
			if (newPosition !== undefined && Object.prototype.toString.call(spritemap[pointer]) === '[object Object]') {
				...
				if (this.context.play) {
                    ...
                    // DEBUG, it will removed after all fixes
                    // var debugDiv = document.getElementById('check');
                    // if (debugDiv != null) {
                    //     debugDiv.innerHTML = pointer + ' ->' + newPosition;
                    // }
				}
			}

//			this.emit(jukebox.Player.events.PLAYING_STARTED);

			this.previouslyPlayed = pointer;
		 ...
		 */
		stop: function(managerCalling) {

			if(!managerCalling && this.isPlaying ){
				this.isPlaying.loop = false;
//				this.emit(jukebox.Player.events.LOOPING_ENDED);
			}
//            document.getElementById('check').innerHTML = pointer;
			...
			if (this.backgroundMusic) {
				...
				this.context.pause();
//				this.emit(jukebox.Player.events.PLAYING_ENDED);
			}
			// clearInterval(this.i);
		...
		HTML5API: {
			...
			/*
			 * {Number}В This method will return the current volume as a {Number}
			 * from 0 to 1.0.
			 */
			getVolume: function() {
