		...
		loop: function() {
			...
			for (var id in this.__players) {
				...
				// Reset / Stop
                    ...
                    //    document.getElementById('loop').innerHTML = playerPosition;
                    // }
        			if (playerPosition === 0) {
                        player.stop(true);
                        return;
					...
					if (playerPosition > player.isPlaying.end) {
						if (player.isPlaying.loop === true) {
                            if (player.loopingSound != null) {
                                player.play(player.loopingSound);
                            } else {
                                player.play('emptyLoop');
                            }
						} else {
							player.stop(true);
                            ...
                            if ( this.features.channels > 1 && player.settings.spritemap.emptyLoop !=  undefined && player.settings.spritemap.emptyLoop.loop ) {
                                if ( player.loopingSound != null ) {
                                    player.play(player.loopingSound);
                                } else {
                                    player.play('emptyLoop');
